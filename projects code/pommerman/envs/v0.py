...
class Pomme(gym.Env):
    ...
    def step(self, actions):
        if actions != [0] * 4 and self._busyWait: # if action is empty we dont care
            obs = self.get_observations()
            for i in range(len(actions)): # is he burning now
                isDeadNow = False
                for f in self._flames:
                    if self._agents[i].position == f.position:
                        isDeadNow = True
                count = 0
                while actions[i] == 0 and not isDeadNow and self._agents[i].is_alive and count < 5:
                    actions[i] = self._agents[i].act(obs[self._agents[i].agent_id], spaces.Discrete(6))
                    count += 1
