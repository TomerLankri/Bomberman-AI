...
class Bomber(object):
    ...
    def __deepcopy__(self, memodict={}):
        result = Bomber.__new__(Bomber)
        memodict[id(self)] = result
        result._game_type = self._game_type
        result.ammo = self.ammo
        result.is_alive = self.is_alive
        result.blast_strength = self.blast_strength
        result.can_kick = self.can_kick
        result.agent_id = self.agent_id
        result.enemies = self.enemies
        result.position = self.position
        result.teammate = self.teammate
